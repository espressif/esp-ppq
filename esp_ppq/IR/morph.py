...
class GraphMerger(GraphCommandProcessor):
    ...
    def _acceptable_command_types(self) -> List[GraphCommandType]:
        return [
            ...
            GraphCommandType.FUSE_RMSNORM,
            GraphCommandType.FUSE_LP_NORMALIZATION,
        ]
    ...
    def process(self, command: GraphCommand) -> Any:
        ...
        if command.command_type == GraphCommandType.FUSE_RMSNORM:
            return self.fuse_rmsnorm()
        if command.command_type == GraphCommandType.FUSE_LP_NORMALIZATION:
            return self.fuse_lp_normalization()
    ...
    def _var_or_alias_is_input(var: Variable, op: Operation) -> bool:
        ...
        for inp in op.inputs:
            if GraphMerger._same_source(var, inp):
                return True
        return False

    def fuse_rmsnorm(self):
        """Detect the decomposed RMS normalization subgraph and fuse it into a
        single ``RMSNormalization`` operation.

        **Before fusion (decomposed ONNX function body):**
        ::

            in_x ── Pow(2) ── ReduceMean ── [Add(eps)] ── Sqrt ── Reciprocal ──┐
        ...
        for op in ops_to_remove:
            ...
            for inp in op.inputs.copy():
                try:
                    ...
                    pass
                if inp.is_parameter and inp is not in_x and inp is not scale_var:
                    inp.value = None
                    op.inputs.remove(inp)
                    if inp.name in graph.variables:
                        graph.variables.pop(inp.name)
    ...
    def fuse_lp_normalization(self):
        """Detect the decomposed Lp normalization subgraph and fuse it into a
        single ``LpNormalization`` operation.
        ...
        **After fusion:**
        ...
        for op in list(self.graph.operations.values()):
            if op.type != 'ReduceL2':
                continue
            self._try_fuse_lp_normalization(op)

    def _try_fuse_lp_normalization(self, reduce_op: Operation) -> None:
        graph = self.graph
        ...
        for old_op in ops_to_remove:
            ...
            for inp in old_op.inputs.copy():
                try:
                    ...
                    pass
                if inp.is_parameter and inp is not in_x:
                    inp.value = None
                    old_op.inputs.remove(inp)
                    if inp.name in graph.variables:
                        graph.variables.pop(inp.name)
