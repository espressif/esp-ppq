...
class TrainedQuantizationThresholdPass(TrainingBasedPass):
    """
    ## Trained Quantization Pass(网络微调过程-TQT)

    Trained quantization optimization, a training-based optimization pass that tunes weights and log2-scale(power-of-2 exponents) for high precision quantization.
    This method follows the idea of training quantization thresholds in log-domain, where:

    scale = 2^(log2_scale)

    instead of directly optimizing scale as in LSQ. This ensures:

        1. Scale remains strictly positive

        2. Scale naturally satisfies POWER_OF_2 constraint

        3. Better numerical stability for power-of-2 quantization


    [This method is proposed by Sambhav R. Jain] (https://arxiv.org/pdf/1903.08066)

    This is an alternative version of TQT, this pass will split your graph into multiple trainable blocks, each blocks will be trained separately.
    Warning: ESP-PPQ Trained Quantization Threshold minimize only the output loss of each block, which means after training the internal results probably goes far away from original.

    ESP-PPQ Trained Quantizaiton Threshold optimization requires 256 ~ 2048 samples for finetuning your network, while the data label is not necessary. All training data are cache in GPU memory or CPU memory for acceleration.
    ...
    Sicne scale_Y = 2 ^ alpha, the formula of calculating the derivatives of y and alpha:

        if y > scale_Y * -128 and y < scale_Y * 127:

        dQuant(y, scale_Y)/dy       = 1

        dQuant(y, scale_Y)/dscale_Y = Quant(y, scale_Y) - y

        dQuant(y, scale_Y)/dalpha = (Quant(y, scale_Y) - y) * ln2 * scale_Y
        ...
        dQuant(y, scale_Y)/dalpha = (-128) * ln2 * scale_Y
        ...
        dQuant(y, scale_Y)/dalpha = (127) * ln2 * scale_Y
    ...
    * block_size(int)
            ...
            ESP-PPQ Trained Quantization Threshold optimization split your graph into blocks at first,
            each block will be finetuned separately.
    ...
    * int_lambda(float)
            ...
            A regularization term for minimize the distance of alpha and round(alpha).

            Default is 0

    ### Usage:

    TQT Optimization Pass should be invoked after Runtime Calibration Pass.

    This pass is inclueded in ESP-PPQ Quantization Setting, you can calling this optimization by:
    ...
    You can manually create this optimization by:
        ...
        from esp_ppq import TrainedQuantizationThresholdPass

        optim = TrainedQuantizationThresholdPass()


    ### Block-partitioning Algorithm

    ESP-PPQ use Block-partitioning algorithm for spliting your graph into blocks, this method is based on graph theory.
    ...
    def __init__(
        self,
        name: str = 'ESP-PPQ TQT Optimization',
        interested_layers: List[str] = [],
        steps: int = 500,
        gamma: float = 0.0,
        is_scale_trainable: bool = True,
    ...
    def finetune(
        ...
        # collect trainable params
        delegators = {}
        trainable_scales, trainable_params = [], []
        for op in block.rps:
            if not isinstance(op, QuantableOperation):
                continue
            if op.is_computing_op or op.type in {'Add', 'Mul'}:  # 独立的 Add Mul 算子也可以训练
            ...
            for cfg, var in op.config_with_variable:
                if cfg.state in {QuantizationStates.ACTIVATED, QuantizationStates.PASSIVE}:
                    delegator = TQTDelegator(config=cfg, var=var, is_scale_trainable=self.is_scale_trainable)
                    trainable_scales.extend(delegator.trainable_tensors())
                    executor.register_quantize_delegate(config=cfg, delegator=delegator)
                    delegators[cfg] = delegator
        ...
        for idx in tqdm(range(steps), desc='# Tuning Procedure '):
            ...
            if hasattr(self, 'int_lambda') and self.int_lambda > 0:
                int_reg = 0.0
                for cfg, delegator in delegators.items():
                    if hasattr(delegator, 'log2_scale') and delegator.is_scale_trainable:
                        alpha = delegator.log2_scale
                        alpha_round = torch.round(alpha)
                        int_reg += torch.mean((alpha - alpha_round) ** 2)

                loss += self.int_lambda * int_reg
