...
| 参数 | 说明 |
| --- | --- |
| `onnx_import_file` | ONNX 模型路径 |
| `espdl_export_file` | `.espdl` 模型导出路径 |
| `calib_dataloader` | 校准数据 |
| `calib_steps` | 校准时使用的 batch 数量 |
| `input_shape` | 模型输入 shape，不包含 batch 维度。例如 `[3, 224, 224]` |
| `evaluate_fn` | 可选的评测函数 |
| `target` | 部署平台 |
| `setting` | AutoQuant 的搜索设置 |
...
`evaluate_fn` 是 AutoQuant 用来比较不同量化配置结果的评测函数。
这是一个可选参数：
- 如果用户提供该函数，AutoQuant 会按照用户提供的 `evaluate_fn` 返回的指标排序，例如分类模型的 top1 / top5，或检测模型的 mAP。
- 如果用户未提供该函数，AutoQuant 会使用默认评测方式进行排序。

默认评测方式如下：首先调用 `graphwise_error_analyse`，在校准数据上计算模型量化的 graphwise SNR 误差，然后取误差最大的 3 层求平均，获取平均值作为结果。

如果需要自定义 `evaluate_fn`，则需要满足以下要求：

- 返回值必须是 `(score, extras)` 二元组
- `score` 必须是有限数字，不能是 `nan`、`inf` 或 `bool`
- `extras` 必须是字典
- `extras` 不能使用 AutoQuant 保留字段：`score`、`hash`、`index`、`folder`、`files`、`strategy`、`params`
...
| `num_of_candidates` | `5` | 最终保留的候选模型数量，也就是 Top-K 的 K。 |
...
| `candidate_filter` | `low_latency_candidate_filter` | Top-K 候选过滤器。默认更偏向保留部署成本较低的结果；如果只想严格按 `score` 排序，可以设置为 `None` |
...
`mobilenetv2.py` 会下载 MobileNetV2 ONNX 和 ImageNet 校准集。`yolo11n.py` 会下载 YOLO11n ONNX、校准集和评测脚本。下载后的文件会放在当前目录下的 `quantize_mobilenetv2/` 或 `quantize_yolo11n/` 中。如果文件已经存在，脚本会直接复用。
