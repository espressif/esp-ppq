...
if os.path.dirname(os.path.realpath(__file__)) == os.path.join(
    os.path.realpath(os.getcwd()), "esp_ppq"
):
    message = (
        "You are importing esp_ppq within its own root folder ({}). "
    )
...
from esp_ppq.IR import (BaseGraph, GraphBuilder, GraphCommand, GraphExporter,
...
from esp_ppq.quantization.analyse import (graphwise_error_analyse,
                                      ...
                                      variable_analyse)
from esp_ppq.quantization.measure import (torch_cosine_similarity,
                                      torch_cosine_similarity_as_loss,
                                      torch_KL_divergence,
                                      torch_mean_square_error, torch_snr_error)
from esp_ppq.quantization.optim import (BiasCorrectionPass, GRUSplitPass,
...
from esp_ppq.quantization.quantizer import (BaseQuantizer, NXP_Quantizer,
                                        PPL_DSP_Quantizer, PPLCUDAQuantizer,
                                        TensorRTQuantizer)
from esp_ppq.scheduler import (AggresiveDispatcher, ConservativeDispatcher,
